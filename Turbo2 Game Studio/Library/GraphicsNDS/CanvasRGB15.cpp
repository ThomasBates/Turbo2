...
void	GraphicObjectRGB15::SetRGB(Color red, Color green, Color blue)
{
...
CanvasRGB15::CanvasRGB15(int width, int height)
{
	_data	= 0;
	_width	= width;
	_height	= height;

	_brush	= new BrushRGB15();
	_pen	= new PenRGB15();
...
void	CanvasRGB15::SetWidth(int width)
{
	if (width != _width)
	{
		u16 *data = 0;
...
void	CanvasRGB15::SetHeight(int height)
{
...
/*
void CanvasRGB15::GetSize(int *width, int *height)
{
	*width	= _width;
	*height	= _height;
}

void CanvasRGB15::SetSize(int width, int height)
{
	if (_width	!= width ||
		_height	!= height)
	{
...
void CanvasRGB15::GetLocation(int *x, int *y)
{
	*x	= _x;
	*y	= _y;
}

void CanvasRGB15::SetLocation(int x, int y)
{
	MoveTo(x, y);
}
*/

void CanvasRGB15::SetPixel(int x, int y, Color color)
{
	if (x >= 0 && x < _width &&
		y >= 0 && y < _height)
	{
...
Color CanvasRGB15::GetPixel(int x, int y)
{
	if (x >= 0 && x < _width &&
		y >= 0 && y < _height)
	{
...
void CanvasRGB15::Clear()
{
	if (_data)
		memset(_data, 0, _width * _height * 2);
}

void CanvasRGB15::Fill()
{
...
void CanvasRGB15::MoveTo(int x, int y)
{
	_x	= x;
	_y	= y;
}

void CanvasRGB15::LineTo(int x, int y)
{
...
void CanvasRGB15::CopyCanvas(ICanvas *canvas, int left, int top, bool transparent)
{
	canvas->CopyToBuffer(_data, _width, _height, left, top, transparent);
}

void CanvasRGB15::CopyToBuffer(void* buffer, bool transparent)
{
	if (transparent)
	{
		for (int i = 0; i < _width * _height; i++)
...
void CanvasRGB15::CopyToBuffer(void* buffer, int bufferWidth, int bufferHeight, int left, int top, bool transparent)
{
	int xMin = MAX(0, -left);
