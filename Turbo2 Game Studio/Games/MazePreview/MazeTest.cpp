...
MazeTest::MazeTest(std::shared_ptr<ITurboDebug> debug) :
	_debug(debug)
{
}
...
std::shared_ptr<ITurboGameState> MazeTest::GameState()
{
	return _gameState;
}

void MazeTest::GameState(std::shared_ptr<ITurboGameState> gameState)
{
	_gameState = gameState;
}

std::shared_ptr<ITurboScene> MazeTest::Scene()
{
	if (_level == nullptr)
	{
		return nullptr;
...
std::shared_ptr<ITurboSceneObject> MazeTest::Player()
{
	return _player;
}
...
void MazeTest::Update(NavigationInfo navInfo)
{
	...
	if (navInfo.Restart || (_level == nullptr))
	{
		...
		_level = std::unique_ptr<ITurboGameLevel>(new MazeTestLevel(_debug, _player, &_userOptions));
