...
std::shared_ptr<ITurboGameState> MazeTestLevel::GameState()
{
...
void MazeTestLevel::GameState(std::shared_ptr<ITurboGameState> gameState)
{
...
std::shared_ptr<ITurboScene> MazeTestLevel::Scene()
{
	if (_subLevel != nullptr)
	{
...
void MazeTestLevel::Update(NavigationInfo navInfo)
{
	_sceneChanged = false;
...
void MazeTestLevel::UpdateMazeOptions(std::shared_ptr<ICubicMazeSceneBuilder>* sceneBuilder, Level00MazeOptions* mazeOptions)
{
