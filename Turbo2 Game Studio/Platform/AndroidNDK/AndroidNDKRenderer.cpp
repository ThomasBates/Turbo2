...
// Ctor
//--------------------------------------------------------------------------------
AndroidNDKRenderer::AndroidNDKRenderer()
...
void AndroidNDKRenderer::Init(const int32_t numX, const int32_t numY,
    ...
    int32_t index = 0;
    SHADER_VERTEX *p = new SHADER_VERTEX[num_vertices_];
...
void AndroidNDKRenderer::UpdateViewport() {
    int32_t viewport[4];
...
// Unload
//--------------------------------------------------------------------------------
void AndroidNDKRenderer::Unload() {
    if (vbo_) {
...
// Render
//--------------------------------------------------------------------------------
void AndroidNDKRenderer::Render() {
    // Bind the VBO
    glBindBuffer(GL_ARRAY_BUFFER, vbo_);
    ...
    SHADER_MATERIALS material = {{1.0f, 1.0f, 1.0f, 10.f},
                                 {0.1f, 0.1f, 0.1f},};
...
// LoadShaders
//--------------------------------------------------------------------------------
bool AndroidNDKRenderer::LoadShaders(SHADER_PARAMS *params, const char *strVsh,
                                     const char *strFsh) {
    //
...
bool AndroidNDKRenderer::LoadShadersES3(
...
// Bind
//--------------------------------------------------------------------------------
bool AndroidNDKRenderer::Bind(ndk_helper::TapCamera *camera) {
    camera_ = camera;
    return true;
}
...
// Helper functions
//--------------------------------------------------------------------------------
std::string AndroidNDKRenderer::ToString(const int32_t i) {
    char str[64];
