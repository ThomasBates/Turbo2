...
#define HELPER_CLASS_NAME \
  "ca/turbobutterfly/ndkhelper/NDKHelper"  // Class name of helper function
...
// Ctor
//-------------------------------------------------------------------------
AndroidNDKGameEngine::AndroidNDKGameEngine(
        android_app* app,
 ...
 * Load resources
 */
void AndroidNDKGameEngine::LoadResources()
{
 ...
 * Unload resources
 */
void AndroidNDKGameEngine::UnloadResources()
{
  renderer_.Unload();
}
 ...
 */
int AndroidNDKGameEngine::InitDisplay(android_app *app)
{
  if (!initialized_resources_)
  {
    gl_context_->Init(_android_app->window);
    LoadResources();
  ...
  else
  {
    ...
    if (EGL_SUCCESS == gl_context_->Resume(_android_app->window))
    {
      UnloadResources();
      LoadResources();
    }
 ...
 */
void AndroidNDKGameEngine::DrawFrame()
{
  float fps;
 ...
 */
void AndroidNDKGameEngine::TermDisplay() { gl_context_->Suspend(); }

void AndroidNDKGameEngine::TrimMemory() {
 ...
 */
int32_t AndroidNDKGameEngine::HandleInput(android_app* app, AInputEvent* event) {
  AndroidNDKGameEngine* eng = (AndroidNDKGameEngine*)app->userData;
 ...
 */
void AndroidNDKGameEngine::HandleCmd(struct android_app* app, int32_t cmd)
{
  AndroidNDKGameEngine* eng = (AndroidNDKGameEngine*)app->userData;
  switch (cmd)
  {
...
// Sensor handlers
//-------------------------------------------------------------------------
void AndroidNDKGameEngine::InitSensors()
{
  sensor_manager_ = ndk_helper::AcquireASensorManagerInstance(_android_app);
  ...
  sensor_event_queue_ = ASensorManager_createEventQueue(
      sensor_manager_, _android_app->looper, LOOPER_ID_USER, NULL, NULL);
}

void AndroidNDKGameEngine::ProcessSensors(int32_t id)
{
...
void AndroidNDKGameEngine::ResumeSensors()
{
...
void AndroidNDKGameEngine::SuspendSensors()
{
...
// Misc
//-------------------------------------------------------------------------
void AndroidNDKGameEngine::SetState(android_app* state)
{
  _android_app = state;
  doubletap_detector_.SetConfiguration(_android_app->config);
  drag_detector_.SetConfiguration(_android_app->config);
  pinch_detector_.SetConfiguration(_android_app->config);
}

bool AndroidNDKGameEngine::IsReady()
{
...
void AndroidNDKGameEngine::TransformPosition(ndk_helper::Vec2& vec)
{
...
void AndroidNDKGameEngine::ShowUI()
{
  JNIEnv* jni;
...
void AndroidNDKGameEngine::UpdateFPS(float fps)
{
  JNIEnv* jni;
  _android_app->activity->vm->AttachCurrentThread(&jni, NULL);
